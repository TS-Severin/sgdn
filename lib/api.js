...
async function fetchGraphQL(query, preview = false) {
  return fetch(
    `https://graphql.contentful.com/content/v1/spaces/${process.env.CONTENTFUL_SPACE_ID}`,
    {
      ...
      next: { tags: ["knowledgeArticle", "landingPage"] },
    }
...
export async function getLandingPage(isDraftMode = false) {
  const landingPage = await fetchGraphQL(
    `query {
              landingPageCollection(limit: 1, preview: ${isDraftMode ? "true" : "false"
    }) {
                items {
  ...
  // Log the landing page data to debug
  console.log("Landing Page Data:", landingPage);

  return landingPage?.data?.landingPageCollection?.items[0];
}